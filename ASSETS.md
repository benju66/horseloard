...
| `public/models/hero/rider-a2.glb` (was rider.glb) | Meshy AI, generated by Ben 2026-08-06 — static rider, no rig (inherits gait from the mount's `chest` bone). NB: the mesh is a *standing archer drawing a bow*, not the seated pose that was prompted — kept because the draw pose reads well from the gameplay camera | Meshy plan grant — as above | No |
...
wolf go dark. Rule: rider carries `emissiveFactor [0,0,0]` and no emissive map; the
Amarok keeps its emissive map and factor exactly as exported. If a future Meshy
