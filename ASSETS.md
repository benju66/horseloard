...
| `public/models/hero/rider.glb` | Meshy AI, generated by Ben 2026-08-06 — static seated rider, no rig (inherits gait from the mount's `chest` bone) | Meshy plan grant — as above | No |
...
lopes at gallop — no idle clip needed. The seated Meshy rider (same day) replaced
the KayKit Knight stand-in: rigid, no skeleton, bone-anchored, so it inherits the
mount's gait for free. Its PBR maps (normal/metallic/emissive, ~7 MB) were stripped
before optimisation — the game lights flat albedo only (ART-BRIEF §3). Seat fit is
tuned by the prop's `scale`/`offset` in `models.json`, judged on device.
