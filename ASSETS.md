...
**Cache rule — rename on every change.** `vercel.json` serves `/models/*` with
`max-age=31536000, immutable`: browsers keep a model file for a year and never
revalidate it. Editing a model in place therefore never reaches installed PWAs
(the all-white rider shipped its fix twice to no effect this way). Any changed
model file must get a new name (`rider.glb` → `rider-a2.glb`) plus its
`models.json` reference updated.
...
**The fox was the pipeline's proving stand-in (2026-08-05) and is now retired
from the default hero** — superseded by the Meshy Amarok below on 2026-08-06.
The file stays in the repo as a known-good rigged quadruped for debugging the
anchor path; if it ever ships in a build again, the CC-BY attribution above
becomes a real obligation.
...
| `public/models/hero/mount-amarok.glb` | Meshy AI, generated by Ben 2026-08-06 — quadruped auto-rig, one walk clip (`Armature\|Unreal Take\|baselayer`) | Meshy plan grant — Ben has ruled the licence question settled | No |
| `public/models/hero/rider-a3.glb` (was rider.glb, rider-a2.glb) | Meshy AI, generated by Ben 2026-08-06 — static rider, no rig (inherits gait from the mount's `chest` bone). NB: the mesh is a *standing archer drawing a bow*, not the seated pose that was prompted — kept because the draw pose reads well from the gameplay camera | Meshy plan grant — as above | No |
| `public/models/meshy/gatehouse-a2.glb` (was gatehouse.glb) | Meshy AI, generated by Ben 2026-08-06 (image-to-3D, faceted regeneration) — static, no rig | Meshy plan grant — as above | No |
| `public/models/meshy/grunt.glb` | Meshy AI, generated by Ben 2026-08-06 — biped auto-rig (24 joints), one walk clip (`Armature\|walking_man\|baselayer`); first painted enemy, replaces the tinted KayKit Skeleton_Minion as `unit-grunt` | Meshy plan grant — as above | No |

**The Amarok is the hero's mount** (the game is retheming away from
"Horse Lord"; a wolf-like mythical mount Meshy *can* auto-rig replaces the
horse it cannot). `hero-mounted` in `models.json` maps its walk clip and
anchors the rider to the `chest` bone. It shipped through
`scripts/optimize-model.mjs` (22.83 MB → 0.44 MB, almost all of it a 22 MB PNG
→ webp). Walk-only is deliberate: the renderer locks the clip's tempo to
ground speed (`WALK_CLIP_SPEED` in `game3d.ts`), so it freezes at rest and
lopes at gallop — no idle clip needed. The Meshy rider (same day) replaced the
KayKit Knight stand-in: rigid, no skeleton, bone-anchored, so it inherits the
mount's gait for free. Its normal/metallic maps were stripped before optimisation —
the game lights flat albedo only (ART-BRIEF §3).

**The emissive trap (2026-08-06, cost one on-device round):** Meshy bakes part of a
model's *look* into its emissive map, with `emissiveFactor` `[1,1,1]`. Strip the
emissive **texture** without zeroing the **factor** and the whole material glows
uniform white — the rider shipped as a featureless white ghost this way. Conversely
the Amarok's frost-white coat partly *is* its emissive map: stripping it makes the
wolf go dark. Rule: keep a Meshy model's emissive map *and* factor together exactly as
exported (both hero models now do), or remove both together — never strip the
map alone. The rider's emissive map is near-black, so for it the pair is
cosmetic; the Amarok's is load-bearing. If a future Meshy
model looks washed-out white or unexpectedly dark, check emissive first.
Seat fit is tuned by the prop's `scale`/`offset` in `models.json`, judged on device.

**The gatehouse replaces the Castle-kit assembly as the gate.** Wired as
`world-gatehouse` (`keepMaterials: true` — it ships its own painted texture) and
placed by `buildGateFromModel` in `src/render/world.ts`, width-locked to the same
`GATE_HALF_WIDTH` footprint as the kit path, which remains as the fallback.
Facing +Z confirmed visually in `inspect.html`: the door-and-banners face points
at the game camera.

**The grunt's emissive was stripped map-and-factor together** (the sanctioned
option from the emissive-trap rule above): Meshy shipped its albedo texture wired
into the emissive slot at factor `[1,1,1]`, which would have added the texture to
itself and washed the unit out. Unlike the Amarok, nothing of the grunt's look
lives in emissive, so removal was safe — verified in `inspect.html` and in-wave.
`unit-grunt` is now a standalone manifest entry (no `base-humanoid`) with
`keepMaterials: true` and only `walk` mapped; other states fall back to
procedural per ART-BRIEF §4. Texture 4096² PNG → 512² webp, 24.36 MB → 0.18 MB.

`-a2` supersedes the first generation (renamed per the cache rule above). The
first was a smooth sculpt whose 188,970 triangles had to be decimated 98.5% to
budget — that melted every edge, and its near-white paint blew out under the day
sun. The faceted regeneration ships at its native 5.9k triangles with **no
simplification at all** (hard edges intact), darker stone, and blue roofs;
texture 2048² JPEG → 512² webp, 0.15 MB all in. The lesson for future structure
generations: ask Meshy for faceted geometry up front — decimating a sculpt down
to a low-poly budget destroys exactly the planes the art direction needs.
