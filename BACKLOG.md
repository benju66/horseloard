...
[ ] **Drafting flattens the curve — decide the correction.** crossroads is out of band at 93%. Two levers: raise `everyNWaves` in perks.json (one number, cheapest), or re-tune enemy scaling now that players compound power within a run (larger, more deliberate, and arguably the honest answer since the curve was calibrated against a game where in-run power was flat). This is a feel call, and the knife-edge warning from the difficulty pass still applies — win rate collapsed 100% → 27% between hp×1.0 and hp×1.4.
[ ] **Does drafting make composition a decision?** The question the slice was built to answer, and it needs a human. The numbers say runs diverge in *power*; whether they diverge in *play* is a phone-and-thumbs question. If the answer is no, the next lever is perks that change rules rather than multiply stats.
