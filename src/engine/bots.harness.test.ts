...
describe.runIf(import.meta.env.MODE === 'balance')('bot matrix', () => {
  ...
  it('reports whether exposure actually multiplies rate', { timeout: 300_000 }, () => {
     ...
     * and the flaw was recorded the day it shipped: both arms saturate on maps
     * 1-2 and both die on 3-4, so it reported "army helps on 0/4 maps" while
     * measuring nothing — an arm that cannot exercise its subject, for the
     * third recorded time.
     *
  ...
  it('reports whether different enemy pools make different builds correct', { timeout: 600_000 }, () => {
    ...
    //
    // Each pool is measured under ITS OWN biome's terrain rule, read from
    // biomes.json — never under the base map's. The base map belongs to one
    // biome, and inheriting its rule taxed every reskin with physics only that
    // biome has: measured, it pushed the green reskin to 0% across the board
    // (swarms and runners under a coverage cut they will never actually meet),
    // which made the control row unreadable in exactly the way a saturated row
    // always is.
     ...
     * of the wave's shape but a different wave (green read 0%). Clamping the
     * counts fixed the floods but let total wave HP drift by the hp ratio of
     * whatever the round-robin happened to swap in (green 100%, iron 0%).
     *
    ...
    for (const [name, biomeId, pool] of POOLS) {
      const ruledMap = { ...data.maps[mapId]!, terrainRule: data.biomes[biomeId]!.terrainRule };
      const poolData = {
        ...botData,
        maps: { ...data.maps, [mapId]: ruledMap },
      ...
      // Top two, not one: a single max-delta label hides the signature. Green
      // and steppe can share a generically-strong top path while differing
