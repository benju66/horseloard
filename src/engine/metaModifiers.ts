import type { Economy, Hero, MapDef, MetaNode, TowersFile } from '../data/schemas';
...
export function applyMetaModifiers(
  ...
  for (const node of nodes) {
    ...
    if (fx.type === 'unlock-ability') {
      unlockedAbilityIds.push(fx.abilityId);
    } else if (fx.type === 'hero-stat') {
      const h = out.hero;
      ...
      else if (fx.stat === 'gateMaxHp') out.map.gate.hp = Math.round(apply(out.map.gate.hp, fx.perRank, fx.mode, rank));
