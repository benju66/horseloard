...
**Painted, not tinted.** These models ship with their own hand-painted low-poly
texture — dull red cloth against steel, leather and skin, with the gentle tonal
variation of a painted tabletop miniature — not one flat palette red. Single-shade
monsters are exactly what this roster exists to replace; the gatehouse-a2 set the
precedent that a painted Meshy texture works under this renderer's lighting, and
the same two constraints carry over from it: **no baked directional lighting** in
the paint (it fights the day/night presets) and **faceted geometry generated
low-poly natively** (decimating a smooth sculpt melts the planes — the first
gatehouse proved it). Every new enemy is wired with `"keepMaterials": true`: the enemy entries in
...
```
A single isolated game character for a minimalist flat-shaded low-poly mobile tower-defence game. Compact sturdy proportions, roughly 3 heads tall, simplified geometric forms with faceted flat planes, no facial features, no thin protruding geometry. Rich hand-painted low-poly style: several distinct colour zones per figure - cloth, leather, metal, skin - with gentle painted tonal variation and worn edges, like a hand-painted tabletop miniature. But no baked directional lighting, no strong painted shadows, no ambient occlusion - the game lights the model itself with a low raking sun. No outlines. The geometry must be authored as faceted low-poly with visible flat planes, never a smooth sculpt. Plain neutral grey background, no scenery, no ground shadow, no props other than the subject. Three-quarter view from clearly above, as though from a camera 55 degrees above the horizon - the silhouette seen from above is what matters most. Bold simple silhouette that stays readable at roughly 40 pixels tall on a phone screen. Elegant and understated rather than detailed or cute.
...
Output: single merged mesh, around 3000 triangles, one material, texture 512x512 or smaller, hand-painted unlit-style colours with no baked directional lighting or ambient occlusion, and no pure white areas. Native faceted low-poly geometry - do not generate a dense smooth sculpt to be decimated later. Y-up orientation, model facing +Z, origin at the feet centred on X and Z. Export as .glb. No PBR maps, no normal map, no metalness or roughness map.
```
